...
const FinalOrderDetails = () => {
  ...
  // Se așteaptă ca location.state să conțină aceste proprietăți:
  // orderId, orderData, paymentMethod, cardType și opțional paymentSummary.
  ...
  // Dacă nu avem paymentSummary transmis, îl construim pe baza informațiilor primite.
  // Pentru card selectat, ne așteptăm ca orderData să conțină o proprietate, de ex.,
  // selectedCardDetails cu detalii precum: { brand, last4, exp_month, exp_year }.
  const computedPaymentSummary =
    paymentMethod === "Card"
      ? cardType === "newCard"
        ? "Plătește cu alt card"
        : orderData.selectedCardDetails
        ? `${orderData.selectedCardDetails.brand} •••• ${
            orderData.selectedCardDetails.last4
          } Expira in ${new Date(
            Number(orderData.selectedCardDetails.exp_year),
            Number(orderData.selectedCardDetails.exp_month) - 1
  ...
  // Folosim paymentSummary din state dacă există, altfel folosim computedPaymentSummary
  const finalPaymentSummary = paymentSummary || computedPaymentSummary;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="p-4 border rounded-md bg-gray-100 shadow-md flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mb-2">{finalPaymentSummary}</p>
          </div>
          <button
            className="mt-2 bg-white border border-gray-300 text-gray-800 px-4 py-2 rounded-full shadow hover:shadow-md hover:bg-gray-100 transition duration-200 text-sm"
